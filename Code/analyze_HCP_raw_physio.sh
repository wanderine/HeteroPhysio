...
do
	...
	echo "Copying data"
	cp $datadir/${subjectID}/unprocessed/3T/tfMRI_${task}_LR/${subjectID}_3T_tfMRI_${task}_LR.nii.gz   bold_HCPphysio_${subject}.nii.gz

	echo -e "\nPerforming slice timing correction using AFNI \n"
	3dTshift -prefix bold_HCPphysio_${subject}_stc.nii.gz -tzero 0 -tpattern @SliceTimingSeconds.txt bold_HCPphysio_${subject}.nii.gz

	echo -e "\nPerforming motion correction using AFNI \n"
	3dvolreg -prefix bold_HCPphysio_${subject}_mc.nii.gz -1Dfile bold_HCPphysio_${subject}_headmotion.1D bold_HCPphysio_${subject}_stc.nii.gz

	echo -e "\nPerforming smoothing using AFNI \n"
	3dmerge -quiet -1blur_fwhm 4.0 -doall -prefix bold_HCPphysio_${subject}_sm.nii.gz bold_HCPphysio_${subject}_mc.nii.gz

	echo -e "\nCreating brain mask using AFNI \n"
	3dAutomask -prefix bold_HCPphysio_${subject}_mask.nii.gz bold_HCPphysio_${subject}_mc.nii.gz

	# Trim data
	fslroi bold_HCPphysio_${subject}_sm.nii.gz bold_HCPphysio_${subject}_sm.nii.gz 10 70 12 80 19 45 0 -1

	# Trim mask
	fslroi bold_HCPphysio_${subject}_mask.nii.gz bold_HCPphysio_${subject}_mask.nii.gz 10 70 12 80 19 45 

	# Calculate motion derivative
	1d_tool.py -infile bold_HCPphysio_${subject}_headmotion.1D -set_nruns 1 -derivative -write bold_HCPphysio_${subject}_headmotion_deriv.1D
	
	# Trim to one slice
	#fslroi bold_HCPphysio_${subject}_sm.nii.gz bold_HCPphysio_${subject}_sm.nii.gz 0 -1 0 -1 27 1 0 -1

	# Trim mask to one slice
	#fslroi bold_HCPphysio_${subject}_mask.nii.gz bold_HCPphysio_${subject}_mask.nii.gz 0 -1 0 -1 27 1
	...
	./HeteroGLM_physio bold_HCPphysio_${subject}_sm.nii.gz -designfiles regressors_HCP_${task}.txt -contrasts contrasts_HCP_${task}.txt -ontrialbeta allbeta_HCPphysio_${task}.txt -gammaregressors gammaintercept_HCP_${task}.txt -ontrialrho allrho_AR10.txt -mask bold_HCPphysio_${subject}_mask.nii.gz -regressmotion bold_HCPphysio_${subject}_headmotion.1D -regressmotionderiv bold_HCPphysio_${subject}_headmotion_deriv.1D  -saveoriginaldesignmatrix -savedesignmatrix -draws 1000 -burnin 1000 -verbose -savefullposterior -arorder 10 -iota 3.0 -taurho 1.0  -regressphysio PNM_${subjectID}_${task}_noslicetiming_evlist.txt 


	mv bold_HCPphysio_${subject}* Results/HCPrawphysio/${task}/stc_mc_smoothed_AR10_nonstationary_iota3.0
	#mv bold_HCPphysio_${subject}* Results/HCPrawphysio/${task}/stc_mc_smoothed_AR10_stationary_iota3.0
